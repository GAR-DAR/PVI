    ...
    <div id="modal-overlay" class="modal-overlay" style="display: flex;">
        <div class="modal-container">
            <div class="modal-header">
                <h2 id="form-title">Edit Student</h2>
            </div>
            <div class="modal-body">
                <form id="student-form" method="POST" action="{{ route('students.update', $student->id) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="profile-photo-section" style="display: flex; justify-content: center; margin-bottom: 20px;">
                        <div class="avatar-upload-container" style="position: relative; cursor: pointer;">
                            <div class="avatar-container" style="width: 100px; height: 100px; border-radius: 50%; overflow: hidden; margin: 0 auto;">
                                <img id="avatar-preview" src="{{ $student->avatar_path ? asset($student->avatar_path) : asset('images/user-sample.png') }}" alt="Profile Avatar" style="width: 100%; height: 100%; object-fit: cover;">
                            </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="group" name="group" required maxlength="10" value="{{ old('group', $student->group_name) }}">
                    </div>

                    <div class="name-row">
                        <div class="form-group">
                            <label for="first_name">First Name:</label>
                            <input type="text" id="first_name" name="first_name" minlength="2" maxlength="50" required value="{{ old('first_name', $student->first_name) }}">
                        </div>
                        <div class="form-group">
                            <label for="last_name">Last Name:</label>
                            <input type="text" id="last_name" name="last_name" minlength="2" maxlength="50" required value="{{ old('last_name', $student->last_name) }}">
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="gender_id" name="gender_id" required>
                            ...
                            <option value="{{ $gender->id }}" {{ old('gender_id', $student->gender_id) == $gender->id ? 'selected' : '' }}>
                                {{ $gender->name }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" id="birthday" name="birthday" required max="{{ date('Y-m-d') }}" value="{{ old('birthday', $student->birthday) }}">
                    </div>
